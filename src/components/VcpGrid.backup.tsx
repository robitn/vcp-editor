...
const VcpGrid: React.FC<VcpGridProps> = ({
  ...
  // Normalize filesystem paths (Windows backslashes -> forward slashes)
  // and pass a normalized absolute path to `convertFileSrc` (do NOT prefix
  // with `file://` to avoid the scheme being percent-encoded into the asset URL).
  ...
  const renderImages = () => {
    return document.images.map((image, index) => {
      ...
      const imageFilePath = vcpResourcesFolder
        ...
        : '';
      const imageSrc = imageFilePath ? toAssetSrc(imageFilePath) : '';
  ...
  const renderButtons = () => {
    return document.buttons.map((button, index) => {
      ...
      const buttonImagePath = isConfigured && vcpResourcesFolder
        ...
        : '';
      const buttonImageSrc = buttonImagePath ? toAssetSrc(buttonImagePath) : '';
